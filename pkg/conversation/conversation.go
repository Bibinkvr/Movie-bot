...
func (c *Conversator) Listen(ctx context.Context, d ...time.Duration) (*gotgbot.Message, error) {
	...
	m, err := Listen(ctx, c.ListenFilter(), c.chatId, c.userId)
	if err == nil && m != nil {
...
func Listen(ctx context.Context, filter filters.Message, chatId, userId int64) (*gotgbot.Message, error) {
	c := make(chan *gotgbot.Message, 1)

	ActiveListeners.Add(NewListener(filter, c, chatId, userId))
...
// Cancel stops all active matching listeners for a given user in a chat.
func Cancel(chatId, userId int64) bool {
	return ActiveListeners.Cancel(chatId, userId)
}
